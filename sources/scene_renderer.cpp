...
void SceneRenderer::draw(const Mesh& mesh) {
...
void SceneRenderer::draw(const Model& model) { drawModelImpl(model, glm::mat4(1.f)); }
...
void SceneRenderer::draw(const Entity& entity) {
    auto* model = AssetManager::getModel(entity.modelId);
...
void SceneRenderer::drawModelImpl(const Model& model, glm::mat4 transform) {
    m_program.setUniformLight(*m_light);
    m_program.setUniformCamera(*m_camera);
